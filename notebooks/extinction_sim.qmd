...
simulate_schedule <- function(policy = c("similarity", "random"),
                              fear0, K, T, eta, seed = NULL) {
...
sim_sim <- simulate_schedule("similarity", fear0, K, params$T, params$eta, seed = params$seed)
...
curves_rand <- map_dfr(seq_len(params$n_random), function(r) {
  sim <- simulate_schedule("random", fear0, K, params$T, params$eta, seed = params$seed + r)
  sim$curve %>% mutate(policy = "Random", run = r)
})

# Summarize random curves
...
p <- ggplot() +
  ...
  labs(
    ...
    subtitle = paste0(
      ...
      ", sigma=", round(sigma, 3),
      " (", params$sigma_scale, "× median distance)"
    )
  ) +
  theme_minimal()
...
```{r}
# ---- Optional: visualize which stimuli were selected (similarity-aware) on the MDS map ----
chosen_ids <- dat$id[sim_sim$chosen]

dat_plot <- dat %>%
  mutate(
    chosen_order = match(id, chosen_ids),
    chosen = !is.na(chosen_order)
  )

ggplot(dat_plot, aes(x = .data[[params$x_cols[1]]], y = .data[[params$x_cols[2]]])) +
  geom_point(alpha = 0.6) +
  geom_point(data = filter(dat_plot, chosen), size = 2) +
  geom_text(
    data = filter(dat_plot, chosen),
    aes(label = chosen_order),
    nudge_y = 0.03,
    size = 3
  ) +
  labs(
    title = "Similarity-aware selected stimuli (order annotated)",
